...
class JobManager:
    def __init__(self):
        ...
        # JOBOCKPIT support start
        self.successful_jobs = []

    def AbortJob(self, job, force=False):
    ...
    def RemoveJob(self, job):
            if job.status == Job.NOT_STARTED:
                    ...
                    if job in self.successful_jobs:
                            self.successful_jobs.remove(job)
    ...
    def jobDone(self, job, task, problems):
        ...
        if problems:
            if hasattr(self.active_job, "keep") and self.active_job.keep:  # JOC
                self.failed_jobs.append(self.active_job)  # JOC
            if not job.onFail(job, task, problems):
                self.errorCB(False)
        else:
            # self.active_job = None  JOC
            if job.onSuccess:
                job.onSuccess(job)
            if hasattr(self.active_job, "keep") and self.active_job.keep:  # JOC
                self.successful_jobs.append(self.active_job)  # JOC
            self.active_job = None
            self.kick()
